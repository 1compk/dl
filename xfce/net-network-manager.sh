...
function extension_prepare_config__install_network_manager() {
	...
	if [[ "${BUILD_DESKTOP}" == "yes" ]]; then
		display_alert "Extension: ${EXTENSION}: Adding extra packages for desktop to image" "network-manager-applet network-manager-ssh network-manager-vpnc nm-connection-editor" "info"
		add_packages_to_image network-manager-applet network-manager-ssh network-manager-vpnc nm-connection-editor
	fi
