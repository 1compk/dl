...
create_gpt_partitions() {
  ...
  echo "Creating EFI partition (8MiB-52MiB) FAT32"
  sudo parted -s "$disk" mkpart efi fat32 8MiB 52MiB
  #sudo parted -s "$disk" set 2 boot on
  #sudo parted -s "$disk" set 2 esp on

  echo "Creating Linux partition (52MiB to 100%) ext4"
  sudo parted -s "$disk" mkpart ext4 ext4 52MiB 100%
...
create_mbr_partitions() {
  ...
  # 2. Create 1st partition: FAT32 (4MiB to 48MiB = 44MiB size)
  echo "Creating Boot partition (4MiB-48MiB) FAT32"
  sudo parted -s "$disk" mkpart primary fat32 4MiB 48MiB
  ...
  echo "Creating Linux partition (48MiB to 100%) ext4"
  sudo parted -s "$disk" mkpart primary ext4 48MiB 100%
